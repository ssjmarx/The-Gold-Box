...
class MessageCollector:
    ...
    def add_message(self, client_id: str, message: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            timestamp_value = message.get("timestamp", time.time() * 1000)
            try:
                if isinstance(timestamp_value, str):
                    # Try to parse string timestamp, fallback to current time if fails
            ...
            logger.error(f"Error adding message for client {client_id}: {e}")
            return False
    ...
    def add_roll(self, client_id: str, roll_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Add timestamp and metadata
            timestamp_value = roll_data.get("timestamp", time.time() * 1000)
            try:
                if isinstance(timestamp_value, str):
                    # Try to parse string timestamp, fallback to current time if fails
                    timestamp_value = int(timestamp_value)
            ...
            logger.error(f"Error adding roll for client {client_id}: {e}")
            return False
    ...
    def get_messages(self, client_id: str, count: int = 15) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Error getting messages for client {client_id}: {e}")
            return []
    ...
    def get_rolls(self, client_id: str, count: int = 15) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Error getting rolls for client {client_id}: {e}")
            return []
    ...
    def clear_client_data(self, client_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error clearing client data for {client_id}: {e}")
            return False
    ...
    def _cleanup_client_messages(self, client_id: str):
        ...
        try:
            ...
            # If still too many messages, keep only the most recent
    ...
    def _cleanup_client_rolls(self, client_id: str):
        ...
        try:
            ...
            # If still too many rolls, keep only the most recent
...
# Global message collector instance
message_collector = None

def get_message_collector() -> MessageCollector:
    """Get or create message collector instance"""
    global message_collector
    if message_collector is None:
...
def get_collector_stats() -> Dict[str, Any]:
    """Get message collector statistics"""
    collector = get_message_collector()
