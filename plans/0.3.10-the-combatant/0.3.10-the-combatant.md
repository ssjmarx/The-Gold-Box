...
## Feature 1: Combat Encounter Management ✅ COMPLETE
...
```
AI → create_encounter(actor_ids) → Backend → WebSocket → Frontend → Foundry API
                                                               ↓
Combat created, initiative rolled → Backend → WebSocket → Frontend → Result → AI

AI → delete_encounter() → Backend → WebSocket → Frontend → Foundry API
                                                          ↓
Combat ended → Backend → WebSocket → Frontend → Result → AI
```
...
- create_encounter with initiative: ✅
- create_encounter (already active): ✅ Returns immediate error (no timeout)
- delete_encounter: ✅
- delete_encounter (no combat): ✅ Returns appropriate error
...
```
AI → advance_combat_turn() → Backend → WebSocket → Frontend → Foundry API
                                                          ↓
Turn advanced → CombatMonitor → Combat state → Backend → AI
```
...
- [x] AI can advance combat turns
- [x] Frontend uses Foundry's native combat API
- [x] Turn order updates correctly
- [x] Combat state reflects new turn
- [x] Appropriate error messages when no combat active
- [x] All tests in function_check.sh pass
- [x] No console errors during turn management
...
```
AI → get_actor_details(token_id, search_phrase) → Backend → WebSocket → Frontend → Foundry API
                                                                              ↓
Token actor data → Backend → WebSocket → Frontend → Result → AI
```
...
- [x] AI can query token-specific actor details
- [x] Actor data includes complete stat block with field names
- [x] Field names match Foundry's internal structure (e.g., "attributes.hp.value")
- [x] Search phrase returns relevant context
- [x] Appropriate error messages for invalid token IDs
- [x] All tests in function_check.sh pass
- [x] No console errors during actor queries
...
```
AI → modify_token_attribute(token_id, attribute_path, value, is_delta, is_bar) → Backend → WebSocket → Frontend → Foundry API
                                                                                                        ↓
Attribute modified → Combat state → Backend → AI
```
...
- [x] AI can modify any token attribute using Foundry's native API
- [x] AI can apply damage and healing to tokens
- [x] Attribute updates work system-agnostically (D&D, Pathfinder, etc.)
- [x] Token bar display updates correctly when is_bar=true
- [x] Relative changes work (is_delta=true) for damage/healing
- [x] Absolute values work (is_delta=false) for setting values
- [x] Appropriate error messages for invalid requests
- [x] All tests in function_check.sh pass
- [x] No console errors during token attribute management
...
- [x] TurnAdvanced delta tracks turn advances
- [x] CombatantChanged delta tracks any token attribute changes
- [x] CombatantChanged delta includes token_id, attribute_path, old_value, new_value
...
6. **Integration Tests**
   - Full combat workflow: create → advance → damage → advance → delete
   - Actor query workflow: create combat → query actor → apply damage → query again
...
**Files Affected:**
- `backend/services/ai_tools/ai_tool_definitions.py` (tool descriptions updated)
- `backend/shared/core/unified_message_processor.py` (role prompts updated)

### Completed Changes

1. **✅ Tool Descriptions Updated** (`ai_tool_definitions.py`)
   - `get_message_history`: Added batching guidance - "Efficient: only call once per turn, not before each tool use."
   - `post_message`: Added batching guidance - "Multiple messages can be batched in a single call (e.g., post a scene description, and an NPC speaking, in seperate messages at same time). Use messages array to batch multiple outputs efficiently."
   - `roll_dice`: Added batching guidance - "Multiple rolls can be batched in a single call (e.g., roll attack and damage together, or roll multiple saving throws simultaneously)."

2. **✅ Role Prompts Updated** (`unified_message_processor.py`)
   - **GM Role**: Added EFFICIENCY instructions - "Batch your actions when possible - combine narrative text with dice rolls, handle multiple NPCs' turns together (e.g., roll all attacks for multiple NPCs in one response, then apply damage to all in another response), and query multiple actors' details before acting."
   - **GM Assistant Role**: Added EFFICIENCY instructions - "Batch related actions when requested - roll multiple dice formulas together, apply effects to multiple actors simultaneously, and combine chat messages logically."
   - **Player Role**: Added EFFICIENCY instructions with player_list support - "Roll multiple dice formulas in a single call when appropriate (e.g., attack roll + damage roll, or multiple saving throws together). Combine your character's description with dice rolls efficiently."

3. **✅ Player List Integration**
   - Player role prompt now accepts {player_list} placeholder
   - Context parameter added to `generate_enhanced_system_prompt()` method
   - All calls to this method updated to pass universal_settings
   - Bug fix: Fixed "name 'context' is not defined" error (2026-01-05)

### Acceptance Criteria

- [x] All existing tool descriptions updated with batching guidance
- [x] All role prompts updated with efficiency instructions
- [x] No setting-specific language (e.g., "goblins") in tool descriptions
- [x] User's specified wording used for advance_combat_turn
- [x] All changes are concise without examples
...
### Integration Tests
- [ ] Full combat workflow (create → advance → damage → advance → delete)
- [ ] Actor query workflow (create → query → damage → query)
- [ ] Multi-combatant encounter with multiple turns
